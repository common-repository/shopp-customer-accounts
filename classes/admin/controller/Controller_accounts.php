...
class Controller_accounts extends AbstractAdminListController {
	...
	public function handleSubmit($data) {
		...
		$do_notify = value_is_true($data['notify-applicant']);
		...
		if ($this->isBulkSubmit()) {
			...
			$do_notify = true; //override for now
		...
		foreach($id_list as $id) {
			switch ($verb) {
				case self::APPROVED: $this->approve($id, $do_notify); break;
				case self::REJECTED: $this->reject($id, $do_notify); break;
			}
		}
	...
	public function createAccountRequest($data) {
		...
		//apply updates to customer object so values stay in form
		$this->updateCustomer($data);
		
		//custom validation filter
		$message = apply_filters('sws-account-request-validate', null, $data);
		if (null != $message) {
			new ShoppError($message, '', SHOPP_COMM_ERR);
			return false;
		}
		...
		$message = $this->checkExistingUsers($data['email']);
		if (false !== $message) {
		...
		if (false === $wpdb->insert($this->table, $data)) {
			$message = apply_filters('sws-customer-queue-insert-error', 'Sorry, but there was an error creating your account. Please contact us directly.');
			new ShoppError($message, '', SHOPP_COMM_ERR );
			return false;
		}
		...
		//check whether to automatically approve all requests
		if (Util::isTrue($ShoppWholesale->Settings->get('require-approval'))) { 

			//show confirmation text and wait for approval
			...
			//This is a bit round-about: we create a request, approve it, and delete it straight-away.
			//However, it is cleaner to always follow the same workflow, even for 'instant' approvals			
			$this->approve($queue_id);
			
			//happy days!
			echo apply_filters('sws-customer-queue-instant-approve', '<h3>Thank you</h3><p>Your account has been created! You can now log in to our store.</p>');
	...
	private function updateCustomer($data) {
		...
		global $Shopp;
		if (empty($Shopp->Order->Customer)) $Shopp->Order->Customer = new Customer();
		if (empty($Shopp->Order->Shipping)) $Shopp->Order->Shipping = new Shipping();
		if (empty($Shopp->Order->Billing)) $Shopp->Order->Billing = new Billing();
		
		$Shopp->Order->Customer->updates($data);
		$Shopp->Order->Shipping->updates($data['shipping']);
		$Shopp->Order->Billing->updates($data['billing']);
	 ...
	 * @throws Exception
	 */
	private function approve($id, $do_notify = true) {
	 ...
	 * @param $request
	 */
	protected function notifyAccept($request) {
		return $this->sendMail($request, 'approval-subject', 'approval-email');
	}
	 ...
	 */
	protected function notifyReject($request) {
		return $this->sendMail($request, 'rejection-subject', 'rejection-email');
	}
	 ...
	 * @param $message
	 */
	protected function sendMail($request, $subject_setting, $message_setting) {
		
		global $ShoppWholesale;
		
		$values = array('{site}' => get_bloginfo());
		foreach ($request as $k=>$v) {
			$values['{'.$k.'}'] = $v;
		}
		
		$subject = $ShoppWholesale->Settings->get($subject_setting);
		$message = $ShoppWholesale->Settings->get($message_setting);		
		
		$subject = str_ireplace(array_keys($values), array_values($values), $subject);
		$message = str_ireplace(array_keys($values), array_values($values), $message);

		return wp_mail($request->email, $subject, $message);
	 ...
	 * @param $id
	 */
	private function reject($id, $do_notify = true) {
		
		$request = $this->loadObject($id);
		$result = $this->delete($id);
		if ($result) {
			$this->notifyReject($request);
		}
