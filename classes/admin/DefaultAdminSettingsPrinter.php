...
class DefaultAdminSettingsPrinter implements IAdminSettingsPrinter {
	...
	function printTextField($args) {
		...
		echo "<input type='text' name='".ShoppWholesale::OPTION_NAME."[$name]' id='$name' value='$value' {$this->printArgs($args)} />\n";
		$this->printFieldHelp($name);

	}

	/**
	 * Print a standard text area field.
	 *
	 * @param $args
	 */
	function printTextAreaField($args) {

		$name = $args['name'];
		$value = $this->Settings->get($name);
		unset($args['name']);

		echo "<textarea name='".ShoppWholesale::OPTION_NAME."[$name]' id='$name' {$this->printArgs($args)}>$value</textarea>\n";
		$this->printFieldHelp($name);
	...
	function printCheckboxField($args) {
		...
		echo "<input type='checkbox' name='".ShoppWholesale::OPTION_NAME."[$name]' id='$name' value='on' $checked {$this->printArgs($args)}/> \n";
	...
	protected function printArgs($args) {
		$s = '';
		foreach ($args as $k=>$v) {
			$s .= " $k='$v' ";						
		}
